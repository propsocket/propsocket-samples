...
residents, leases), change the `ENTITY` constant in `main.py`.
...
On any non-2xx response the program prints the API error `title`, `detail`,
and `request_id` (quote it to support) and exits non-zero.
